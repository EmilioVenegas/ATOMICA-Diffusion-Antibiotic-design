...
try:
    ...
    DRUGLIKE_ATOMS_DECODER = dataset_params['crossdock']['atom_decoder']
    DRUGLIKE_ATOM_TO_IDX = {atom: i for i, atom in enumerate(DRUGLIKE_ATOMS_DECODER)}
    
    # Create mapping from ATOMICA atom index to Druglike atom index
    # ATOMICA vocab: [PAD, MASK, GLOBAL] + ATOMS
    num_atomica_atoms = len(VOCAB.idx2atom)
    ATOMICA_TO_DRUGLIKE_MAP = np.full(num_atomica_atoms, -1, dtype=np.int64)
    
    for i, atom_symbol in enumerate(VOCAB.idx2atom):
        # ATOMICA symbols might be mixed case? format_atom_element converts to Title case (e.g. 'Cl')
        # DRUGLIKE symbols are Title case (e.g. 'Cl', 'C')
        if atom_symbol in DRUGLIKE_ATOM_TO_IDX:
            ATOMICA_TO_DRUGLIKE_MAP[i] = DRUGLIKE_ATOM_TO_IDX[atom_symbol]
            
    # Also define atomica_block_encoder
    atomica_block_encoder = VOCAB.symbol2idx
    
    # And atomica_atom_encoder (used in process_expert_atomica.py)
    atomica_atom_encoder = VOCAB.atom2idx

except ImportError:
    print("WARNING: Could not import ATOMICA.data.pdb_utils. ATOMICA constants not initialized.")
    ATOMICA_TO_DRUGLIKE_MAP = None
    DRUGLIKE_ATOMS_DECODER = None
    atomica_block_encoder = None
    atomica_atom_encoder = None
