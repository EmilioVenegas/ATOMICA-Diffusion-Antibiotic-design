...
class RLLoop:
    ...
    def run_pipeline(self, input_path: Path, output_path: Path, 
        ...
        # Score with ADMET-AI
        if self.verbose:
            print(f"\nScoring {len(df)} molecules with ADMET-AI...")
        ...
        if admet_scores is not None and not admet_scores.empty:
            ...
            print("Warning: ADMET scoring failed, using placeholder scores")
            df['composite_score'] = 0.5
        
        # Rank by score
        df = self.rank_by_score(df)
        ...
        if save_top_sdf and not df.empty:
            ...
            mol_id_to_mol = {}
            for file_name, file_mols in mol_dict.items():
                for i, mol in enumerate(file_mols):
                    mol_id = f"{Path(file_name).stem}_{i}"
    ...
    def save_top_molecules_sdf(self, df: pd.DataFrame, molecules_dict: dict, 
        ...
        Args:
            df: DataFrame with ranked results (must have 'molecule_id' and 'smiles' columns)
        ...
        for idx, row in top_df.iterrows():
            ...
            if mol_id in molecules_dict:
                ...
                if mol is not None:
                    ...
                    if 'rank' in row:
                        mol.SetProp('rank', str(row['rank']))
                    if 'composite_score' in row:
                        mol.SetProp('composite_score', f"{row['composite_score']:.4f}")
                    if 'smiles' in row:
                ...
                if self.verbose:
                    print(f"  Warning: Could not find molecule: {mol_id}")
        
        writer.close()
        
        if self.verbose:
            print(f"✓ Saved top-{saved_count} molecule(s) to {output_path}")
